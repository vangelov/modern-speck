...
void main() {
    ...
    float strength = uDOFStrength * (uRes.y / 768.0);

    float depth = texture(uDepth, coord).r;
    float range = uDOFPosition - depth;
    float scale = abs(range);

    vec4 sampl = texture(uColor, coord);
    float count = 1.0;

    for (int i = 0; i < 64; i++) {
        vec2 p = coord + samples[i] * scale * 64.0 * strength * invRes;
        float d = texture(uDepth, p).r;
        float s = abs(uDOFPosition - d);
        sampl += texture(uColor, p) * s;
        count += s;
    }

    fragColor = sampl / count;
}
`;
